...
namespace WebApplication1.Data
{
    public class VariableObject : IDisposable
    {
        ...
        public CurrentVariableValue FillCurrentValue()
        {
            var result = new CurrentVariableValue();
            if (_lastValue == null && _partVariable != null)
            {
                result.VariablePath = _partVariable.path;
                result.LastUpdated = _lastValue.event_timestamp;
                result.CurrentValue = ConvertValueToString();
            }       
            return result;
        }
        ...
        private string ConvertValueToString()
        {
            ...
            switch (observerType)
            {
                case ObserverType.BOOLEAN_OBSERVER:
                    return _lastValue.value_bool.ToString();
                case ObserverType.FLOAT_OBSERVER:
                    return _lastValue.value_float.ToString();                    
                case ObserverType.ENUM_OBSERVER:
                    return _lastValue.value_enum.ToString();                    
                case ObserverType.INT_OBSERVER:
                    return _lastValue.value_int.ToString();                   
                case ObserverType.STRING_OBSERVER:
                    return _lastValue.value_string.ToString();                    
                case ObserverType.WORD_OBSERVER:
                    return _lastValue.value_word.ToString();                
            }
            return "";
        }
        ...
        private void OnGetObserverValues(IObservableList<ObserverDto> obj)
        {
            foreach (ObserverDto observerDto in obj)
            {
                ...
                _mcsPartValue.event_timestamp = DateTime.Now.ToUniversalTime();
